...
# pdir <- '/Users/wenpinhou/Dropbox/pardoll/nsclc/pseudotime_treg/mypt/plot/'
...
rdir <- '/home/whou10/scratch4/whou10/pardoll/nsclc/pseudotime_treg/mypt/res_tde/'
pdir <- '/home/whou10/scratch4/whou10/pardoll/nsclc/pseudotime_treg/mypt/plot/'
...
library(ggplot2)
library(RColorBrewer)
pdf(paste0(pdir, 'dm_patient_alltreg.pdf'), width = 4, height = 3)
ggplot(data = meta) +
...
pdf(paste0(pdir, 'dm_patient_facet_alltreg.pdf'), width = 6, height = 4.5)
ggplot(data = meta) +
  geom_point(aes(x = DC1, y = DC2, color = patient_id), size = 0.1) +
  theme_classic() + xlab('DM1') + ylab('DM2') + facet_wrap(~patient_id)
dev.off()

pdf(paste0(pdir, 'dm_celltype_alltreg.pdf'), width = 4.8, height = 3)
ggplot(data = meta) +
...
pdf(paste0(pdir, 'dm_patient_c0_c7_facet.pdf'), width = 6, height = 4.5)
ggplot(data = meta[cell.select, ]) +
  geom_point(aes(x = DC1, y = DC2, color = patient_id), size = 0.5) +
  theme_classic() + facet_wrap(~patient_id)
dev.off()
...
pdf(paste0(pdir, 'dm_c0_c7_selected_patient_facet.pdf'), width = 6, height = 4.5)
ggplot(data = meta[cell.select, ]) +
  geom_point(aes(x = DC1, y = DC2, color = patient_id), size = 0.5) +
  theme_classic() + facet_wrap(~patient_id)
dev.off()
...
pdf(paste0(pdir, 'dm_cluster.pdf'), width = 3, height = 2)
...
pdf(paste0(pdir, 'dm_pseudotime_path1.pdf'), width = 3, height = 2)
...
r[rownames(tmp[tmp[,1]=='R',])] <- 1
design = cbind(design, response = r[rownames(design)])
input = list(expr = saver[,p1], cellanno = cellanno[p1,], design = design, pseudotime = pt)
saveRDS(input, paste0(ddir, 'C0_to_C7_lamian_input.rds'))
