...
write.csv(cbind(res, cluster = Res$cluster[allg]), paste0(pdir, 'testtime_differential_genes.csv'))
...
pdf(paste0(pdir, 'hm_GO_term.pdf'), width = 7.2, height = 7)
...
print(plotFitHm(Res, showRowName = T, subsampleCell = F, cellWidthTotal = 1000, cellHeightTotal = length(Res$cluster) * 10))
dev.off()


gene <- c('GZMA', 'CCL5', 'NKG7', 'GZMK', 'IL6R', 'SELL', 'CD74', 'CCR7', 'IFNGR2', 'TCF7', 'HLA-DPA1', 'IL2RA')
